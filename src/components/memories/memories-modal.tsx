...
export function MemoriesModal({
  ...
  return (
    ...
    >
<DialogContent className="bg-gradient-to-br from-white to-pink-50 w-full max-w-[90vw] sm:max-w-md p-0 overflow-hidden rounded-xl shadow-lg border border-pink-100">
  <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-pink-300 to-pink-500" />

  <DialogHeader className="px-4 pt-5 pb-3 sm:px-6">
    <div className="flex items-center gap-2 mb-1">
      <Heart className="w-5 h-5 text-pink-500 fill-pink-500" />
      <DialogTitle className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-pink-500 to-pink-700 bg-clip-text text-transparent">
        Nova Memória
      </DialogTitle>
    </div>
  ...
  <form onSubmit={handleSubmit(handleCreateMemory)} className="px-4 sm:px-6 pb-6">
      ...
      <div className="space-y-2">
        ...
        {!previewUrl ? (
            ...
            <Controller
              ...
              render={({ field }) => (
                <div className="relative">
                  <label
                    htmlFor="file-upload"
                    className="flex flex-col items-center justify-center w-full h-36 sm:h-40 border-2 border-dashed border-pink-200 rounded-lg cursor-pointer bg-pink-50/50 hover:bg-pink-100/50 transition-all duration-300 group"
                  >
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                      <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-pink-100 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform duration-300">
                        <Camera className="w-6 h-6 sm:w-8 sm:h-8 text-pink-500" />
                      </div>
          ...
          <div className="relative mt-2 rounded-lg overflow-hidden border border-pink-200 shadow-sm group">
            <img src={previewUrl || "/placeholder.svg"} alt="Preview" className="w-full h-40 sm:h-52 object-cover" />
    ...
    <DialogFooter className="mt-8 flex flex-col-reverse sm:flex-row gap-3 sm:justify-end">
      <Button
        ...
        className="w-full sm:w-auto border-pink-200 text-pink-600 hover:bg-pink-50 rounded-full px-5 transition-all duration-200"
      >
        Cancelar
      </Button>
      <Button
        type="submit"
        disabled={isSubmitting}
        className="w-full sm:w-auto bg-gradient-to-r from-pink-500 to-pink-600 hover:from-pink-600 hover:to-pink-700 text-white rounded-full px-6 shadow-md hover:shadow-lg transition-all duration-200"
      >
        {isSubmitting ? (
